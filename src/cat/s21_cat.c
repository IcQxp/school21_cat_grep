...
int parser(int argc, char **argv, Flags *flag) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (argv[i][0] == '-') {
      ...
      for (size_t j = 1;
           (j < arg_len) && opt_long == CLEAR && er_stat_parse == OK; ++j) {
        switch (argv[i][j]) {
        case '-':
          if (strcmp(opt_phrase, "help") == 0) {
            ...
            puts("Try './s21_cat --help' for supporting information.");
            er_stat_parse = ERROR;
          }
          break;
        case 'b':
...
int file_handler(char *path, Flags *flag) {
  ...
  if (0 != (errcode = fopen_s(&file_ptr, path, "r"))) {
    ...
    // fprintf(stderr, "s21_cat: %s: %s\n", path, strerror_s(buf, sizeof(buf),
    // errcode));
...
void b_handler(FILE *path_file, const int *c, Flags *flag, unsigned *num_str,
...
void s_opt_handler(FILE *path_file, int *next_char, int *c, Flags *flag,
                   unsigned *num_str, const char *flag_position) {
  if (*next_char == '\n') {
    if (flag->e) {
      printf("$");
    }
...
void v_handler(int *c, Flags *flag) {
  if (flag->v) {
    ...
    else if (*c == '\r') { // Проверка на символ \r
      putchar('^');
      *c += 64;
    }
  }
